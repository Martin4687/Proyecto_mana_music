...
function Ventas() {
  ...
  try {
    for (const item of carrito) {
      ...
      const inventarioRes = await axios.get(`${API_URL}/inventarios/`);
      
      // Buscar el inventario específico de este producto
      const inventario = inventarioRes.data.find(
        inv => (inv.producto_info?.id_producto || inv.id_producto) === item.producto.id_producto
      );
      ...
      // Si estamos editando, sumar el stock de la venta original
  ...
  const handleGuardarVenta = async () => {
    ...
    try {
      ...
      if (modoEdicion) {
        ...
        await axios.delete(`${API_URL}/detalle-venta/?venta=${ventaActual.id_venta}`);
      } else {
        // CREAR NUEVA VENTA
  ...
  try {
    ...
    // Buscar el inventario de este producto
    const inventario = inventarioRes.data.find(
      inv => (inv.producto_info?.id_producto || inv.id_producto) === productoId
    );
    
    if (!inventario) {
      console.warn(`No hay inventario para producto ${productoId}`);
      return;
    }
  ...
  const abrirModalDetalle = async (venta) => {
    try {
      const detallesRes = await axios.get(`${API_URL}/detalle-venta/?venta=${venta.id_venta}`);
  ...
  try {
    const detallesRes = await axios.get(`${API_URL}/detalle-venta/?venta=${ventaActual.id_venta}`);
    const usuario = JSON.parse(localStorage.getItem('user'));
    
    // Obtener TODOS los inventarios una sola vez
